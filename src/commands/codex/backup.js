...
export async function createBackup({
  ...
  const copiedDbFiles = [];
  for (const dbPath of stateDbPaths(codexHome)) {
    const relativeDbPath = path.relative(codexHome, dbPath);
    for (const suffix of ["", "-shm", "-wal"]) {
      const sourcePath = `${dbPath}${suffix}`;
      const relativeFilePath = `${relativeDbPath}${suffix}`;
      const copied = await copyIfPresent(sourcePath, path.join(dbDir, relativeFilePath));
      if (copied) {
        copiedDbFiles.push(relativeFilePath);
      }
    }
  }
  ...
  if (restoreDatabase) {
    ...
    for (const suffix of ["", "-shm", "-wal"]) {
      for (const fileName of [
        ...
        path.join("sqlite", `${DB_FILE_BASENAME}${suffix}`)
      ]) {
